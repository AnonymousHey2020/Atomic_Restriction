  ...
  public:
    ...
    void makeWrapper(Module &M)
    {
        ...
        for (auto FI = M.begin(); funNum > 0; FI++)
        {
            ...
            FunctionCallee nFCe = M.getOrInsertFunction(("wrapper_by_compiler_" + FI->getName().str() + "_" + M.getName().str()), FI->getFunctionType(), FI->getAttributes());
    ...
    void initSSWrapperMap(Module &M)
    {
        for (auto FI = M.begin(); FI != M.end(); FI++)
        {
            if(FI->size() == 0)
            {
                if (FI->getName().str().find("llvm") != string::npos)
                    continue;
                errs() << FI->getName() << "\n";
                wrapperMap[FI->getName().str()] = NULL;
            }
        }
    }
    ...
    bool runOnModule(Module &M) override {
        LLVMContext &ctx = M.getContext();
        if (needInstrument == false)
        {
            FunctionCallee noInstrument = M.getOrInsertFunction("noInstrument", Type::getVoidTy(ctx));
            return false;
        }
